...
def build_primitive_prompt_addendum(bundle: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        "  1. MINIMUM 3 SEPARATE PAGES with their own route and page.tsx:\n"
        ...
        "     - app/about/page.tsx OR app/integrations/page.tsx (fully built, not a stub)\n"
        "     A single-page site is an AUTOMATIC FAILURE.\n\n"
        "  2. PROJECT FOLDER: name the project folder after the company (lowercase,\n"
        "     hyphens). 'lattice-site' or 'my-app' = failure. 'vantage' or 'meridian' = correct.\n\n"
        "  3. NO CONTINUOUS CSS GRADIENTS on section backgrounds. No linear-gradient(),\n"
        "     no radial-gradient(), no aurora keyframes. Use STEPPED GRADIENT PANELS\n"
        "     instead (the background.stepped-gradient-panels primitive — a flex row of\n"
        "     10-16 colored divs creating abrupt color transitions between strips).\n\n"
        "  4. NO rounded-xl or rounded-2xl on cards/containers. Sharp corners (rounded-none\n"
        "     or rounded-sm) only. The only exception is small pills/badges.\n\n"
        "  5. VISUAL BOLDNESS: dense grids, color variation, stepped gradients, mixed\n"
        "     background treatments (white → gradient → dark → image → colored). Every\n"
        "     section must look COMPLETELY DIFFERENT from its neighbors.\n\n"
        "  6. Tailwind v4: define all custom colors in @theme inline { } in globals.css,\n"
        "     NOT in :root. :root vars don't generate utility classes in Tailwind v4.\n\n"
        "## COMPLIANCE SUMMARY\n"
        "Before outputting code, confirm:\n"
        "  • 3+ separate pages exist (home + pricing + about/integrations)\n"
        "  • Every primitive above is implemented (not skipped)\n"
        "  • Zero continuous CSS gradients (use stepped panels instead)\n"
