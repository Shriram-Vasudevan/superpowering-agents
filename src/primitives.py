...
def build_primitive_prompt_addendum(bundle: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        "  STEP 7. After building, start dev server. Call superpower_check_layout\n"
        "          on EVERY page (/, /pricing, /about, /integrations). Fix issues.\n"
        "          Use ToolSearch to fetch the tool schema if needed.\n"
        "  STEP 8. Verify every selected primitive is actually imported and used in code.\n"
        "          If you selected a primitive but never imported it — go back and use it.\n"
        "  STEP 9. Only THEN declare complete.\n\n"

        "## NON-NEGOTIABLE BUILD REQUIREMENTS\n\n"

        "  PAGES: 4 separate pages minimum, each SUBSTANTIAL:\n"
        "     - app/page.tsx: home — 12+ sections, long scroll\n"
        "     - app/pricing/page.tsx: full pricing — 8+ sections (tiers, comparison,\n"
        "       FAQ, testimonial, CTA — not just 3 cards and a footer)\n"
        "     - app/about/page.tsx: about — 8+ sections (NO generic timeline — tell\n"
        "       the story differently: investor logos, team grid with real photos,\n"
        "       office culture, values as a bold visual layout, press mentions)\n"
        "     - app/integrations/page.tsx: integrations — 6+ sections (grid of\n"
        "       integration cards with real logos/detail, categories, API preview)\n\n"
        ...
        "  VISUAL QUALITY — prioritize these over speed:\n"
        "     - Do NOT pick 'safe' primitives. Pick the discovery_spotlight ones that\n"
        "       make output feel CUSTOM. If you skip @paper-design/shaders-react,\n"
        "       react-parallax-tilt, split-type, horizontal-scroll — you're being lazy.\n"
        "     - Every section must evoke a FEELING. If a section could be described as\n"
        "       'heading + paragraph + card grid' it is too generic. Redesign it.\n"
        "     - No two CTAs across the site should look the same.\n"
        "     - No logo-strip marquee as social proof. Use testimonial quotes, real\n"
        "       customer stories, or metric callouts instead.\n"
        "     - No generic timeline for company history. Tell the story visually —\n"
        "       oversized year numbers, full-bleed milestone images, pull quotes.\n"
        "     - No CountUp-number-grid as the only social proof pattern. Mix it up.\n"
        "     - Every page hero must look COMPLETELY DIFFERENT from every other page hero.\n"
        "     - Think about what FEELING the site should evoke. Then design to that feeling.\n"
        "       Grids of cards evoke nothing. A bold statement + rich visual does.\n\n"
