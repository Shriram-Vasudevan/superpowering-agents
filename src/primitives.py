...
def build_primitive_prompt_addendum(bundle: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        "  • 4 separate substantial pages\n"
        "  • Every selected primitive is imported and visibly used\n"
        "  • superpower_design_review called and PASSED\n"
        "  • superpower_check_layout run on every page\n"
        "  • Every section justifies its existence — if it's boring, cut it\n"
        "  • Each background treatment used max ONCE per page (variety by construction)\n"
        "  • FAQ sections are interactive accordions, not static text\n"
        "  • Marquees have 15+ items, no visible seam/gap\n"
        "  • Horizontal scroll sections hide native scrollbar\n"
        "  • Pages scroll to top on navigation\n"
        "  • Zero continuous CSS gradients, zero rounded-xl on containers\n"
        "  • Brand name is text only, no two page heroes look the same\n"
        "  • QUALITY FLOOR: if ANY section feels phoned in — the page fails.\n"
        "    A page with 7 strong sections and 2 boring ones is worse than\n"
        "    7 strong sections alone. Cut the boring ones or make them powerful.\n"
        "If any fails — go back and fix it. You are the CEO. Ship quality."
    )
