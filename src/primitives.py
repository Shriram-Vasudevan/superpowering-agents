...
def build_primitive_prompt_addendum(bundle: dict[str, Any]) -> str:
    ...
    lines.append(
        "\n## MANDATORY WORKFLOW — follow this EXACT sequence of numbered steps\n"
        "You MUST complete every step. Skipping any step = failure.\n\n"
        "  STEP 1. Study reference images: Open each image_path with your Read tool.\n"
        "          Extract exact colors, border-radius, typography, layout from each.\n"
        "  STEP 2. Study reference_visual_specs: Read the structured CSS parameters.\n"
        "          These override your instincts. Copy them.\n"
        "  STEP 3. You already have primitives (above). Run the npm install command.\n"
        "  STEP 4. Call superpower_images 3+ times for real Unsplash photos.\n"
        "  STEP 5. Write a detailed ExecPlan (section-by-section breakdown for every page).\n"
        "  STEP 6. Build the full site (or spin up a sub-agent to build it).\n"
        "  STEP 7. Call superpower_design_review with descriptions of each section.\n"
        "          If it returns FAIL or NEEDS WORK — revise and rebuild.\n"
        "  STEP 8. Start the dev server. Call superpower_check_layout on EVERY page\n"
        "          (/, /pricing, /about, /integrations). Fix ALL flagged issues.\n"
        "  STEP 9. Only THEN declare the build complete.\n\n"
        "  You MUST use ToolSearch to fetch superpower_design_review and\n"
        "  superpower_check_layout schemas if you haven't already. These tools exist.\n"
        "  Not knowing their params is not an excuse to skip them.\n\n"
        "## NON-NEGOTIABLE BUILD REQUIREMENTS\n"
        "These are as mandatory as the primitives above. Violating any = failure.\n\n"
        "  1. MINIMUM 4 SEPARATE PAGES with their own route and page.tsx:\n"
        "     - app/page.tsx (home — 10+ sections, long scroll, visually varied)\n"
        "     - app/pricing/page.tsx (full pricing with tiers and feature comparison)\n"
        "     - app/about/page.tsx (team, mission, company history — fully built)\n"
        "     - app/integrations/page.tsx (dedicated integrations page with detail cards)\n"
        "     A single-page or two-page site is an AUTOMATIC FAILURE. Each page must be\n"
        "     substantial — not a stub. Integrations MUST be its own page, not just a\n"
        "     section on the home page.\n\n"
        "  2. PROJECT FOLDER: name the folder after the company in lowercase with hyphens.\n"
        "     'lattice-site' or 'my-app' = failure. 'vantage' or 'meridian' = correct.\n\n"
        "  3. NO CONTINUOUS CSS GRADIENTS on section backgrounds. No linear-gradient(),\n"
        "     no radial-gradient(), no aurora keyframes for backgrounds. Use STEPPED\n"
        "     GRADIENT PANELS instead (flex row of 12-20 colored divs with abrupt color\n"
        "     transitions). Use stepped gradients on 2-3 sections MAX — not every section.\n"
        "     Use MORE color shades (16-20 strips) for richer transitions.\n\n"
        "  4. NO rounded-xl or rounded-2xl on cards/containers. Sharp corners only.\n\n"
        "  5. BRAND/COMPANY NAME: use BOLD TEXT ONLY for the brand name in the navbar.\n"
        "     Do NOT put a Tabler icon or any icon next to the company name. Do NOT create\n"
        "     a fake monogram logo (like a 'V' in a circle). Just bold typography.\n\n"
        "  6. VISUAL BOLDNESS: dense grids, color variation, stepped gradients, mixed\n"
        "     background treatments. Every section must look DIFFERENT from neighbors.\n"
        "     Use more animations (scroll-triggered, hover, entrance). Be overstimulating.\n\n"
        "  7. Tailwind v4: define all custom colors in @theme inline { } in globals.css,\n"
        "     NOT in :root. :root vars don't generate utility classes in Tailwind v4.\n\n"
        "  8. AFTER BUILDING: run superpower_check_layout on localhost to verify the site.\n"
        "     This is NOT optional. Start the dev server, call the layout check tool on\n"
        "     each page, fix any issues found. Do not finish without running this.\n\n"
        "## COMPLIANCE SUMMARY\n"
        "Before outputting code, confirm:\n"
        "  • 4 separate pages exist (home + pricing + about + integrations)\n"
        "  • Every primitive above is implemented (not skipped)\n"
        "  • Zero continuous CSS gradients (use stepped panels, 2-3 sections max)\n"
        "  • Zero rounded-xl on containers\n"
        "  • Brand name is bold text only — no icon logos, no monogram circles\n"
        "  • All motion primitives produce visible animations\n"
        "  • Colors defined in @theme inline { }, not :root\n"
        "  • superpower_check_layout has been run on the built site\n"
        "If any fails — fix it."
    )
